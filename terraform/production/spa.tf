...
# Production: serves on trigpointing.uk (main production domain)
# Legacy server is being decommissioned
module "spa_ecs_service" {
  ...
  alb_rule_priority    = 990 # Default catch-all for main domain (replaces legacy nginx proxy)
  host_headers         = ["trigpointing.uk", "www.trigpointing.uk"]
  path_patterns        = null # Match all paths (serves from root)
  create_listener_rule = true # Create simple forward rule (no OIDC auth for production)
