...
threads). Outputs land in the shared dirs:
    ...
    step/driver.step        nominal master (single solid)
    stl/driver.stl          printable mesh
...
def run(*, threads: bool = True, skip_stl: bool = False) -> None:
    ...
    t0 = time.time()
    master = build_driver()
    validate(master, "driver (master)")
    print(f"driver: master volume={master.volume:.0f} mm^3 valid "
          f"({time.time()-t0:.1f}s)")

    step_path = STEP_DIR / "driver.step"
    master.label = "Driver"
    ...
    if not skip_stl:
        tv = time.time()
        stl_path = STL_DIR / "driver.stl"
        note = export_watertight_stl(master, stl_path, "driver")
