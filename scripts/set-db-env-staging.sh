#!/bin/bash
# Set database environment variables for staging from AWS Secrets Manager.
#
# Usage: source scripts/set-db-env-staging.sh
#
# Prerequisites:
...
#   # or
#   . scripts/set-db-env-staging.sh

# Note: Don't use 'set -e' in sourced scripts - it persists in the calling shell

SECRET_NAME="fastapi-staging-postgres-credentials"
REGION="eu-west-1"

echo "Fetching staging database credentials from AWS Secrets Manager..."
...
echo "✓ Staging database environment configured:"
