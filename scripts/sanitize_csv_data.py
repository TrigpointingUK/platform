...
class CSVSanitizer:
    ...
    def sanitize_value(value: str, column_name: str = "") -> str:
        ...
        # E.g., "3207.0" -> "3207"
        if value and value.endswith('.0') and value.replace('.', '').replace('-', '').isdigit():
            value = value[:-2]
    ...
    def sanitize_csv_file(self, csv_file: Path, dry_run: bool = False):
        ...
        try:
            with open(csv_file, 'rb') as f:
                ...
                import re
                time_overflow = bool(re.search(r'\b([2-9]\d\d+|[3-9]\d):\d{2}:\d{2}\b', sample))
            ...
            with open(csv_file, 'r', encoding='utf-8', errors='replace') as infile, \
                ...
                for row in reader:
                    ...
                    for key, value in row.items():
                        ...
                        if value and (
                            '\x00' in value or
                            value in ('0000-00-00', '0000-00-00 00:00:00') or
                            (' days ' in value and ':' in value) or
                            ('time' in key.lower() and ':' in value and CSVSanitizer._has_time_overflow(value))
                        ):
                            row_had_issues = True
                        ...
                        if value == '':
                            value = None  # This will be written as unquoted empty = NULL
