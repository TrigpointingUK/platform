...
class TestUpdateAllTrigstats:
    """Tests for the update_all_trigstats function."""

    def test_update_all_trigstats(self, db: Session):
        """Test that update_all_trigstats updates stats for all trigs with logs."""
        trig1 = _create_test_trig(db)
        trig2 = _create_test_trig(db)
        user = _create_test_user(db)

        # Create logs directly (avoid existing trigstats updates)
        log1 = TLog(
            trig_id=trig1.id,
            user_id=user.id,
            date=date(2023, 6, 1),
        ...
        # Clear any existing trigstats
        db.query(TrigStats).filter(TrigStats.id.in_([trig1.id, trig2.id])).delete(
        ...
        # Should have updated at least our two trigs
        assert updated_count >= 2
        assert error_count == 0

        # Verify stats exist for both trigs
        stats1 = db.query(TrigStats).filter(TrigStats.id == trig1.id).first()
        stats2 = db.query(TrigStats).filter(TrigStats.id == trig2.id).first()

        assert stats1 is not None
        assert stats1.logged_count == 1
        assert stats2 is not None
        assert stats2.logged_count == 1
